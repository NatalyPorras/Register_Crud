...
const Index = () => {
    ...
    const initialValues = dataParse[Number(id)]
    const onSubmit = async values => {
        const updateData= dataParse
        updateData[id] = values
        window.localStorage.setItem('items', JSON.stringify(updateData));
        await router.history.push(routesPath.LIST_REGISTER)
    }
    return (
              <div className={classes.form}>
        <Form onSubmit={onSubmit} validate={formValidator} initialValues={initialValues}>
      {formProps => {
